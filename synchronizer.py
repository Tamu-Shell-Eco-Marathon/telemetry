...
class SyncApp:
    def __init__(self, root):
        ...
        # --- UI ELEMENTS ---
        tk.Label(
            root, text="Shell Eco-marathon Telemetry Sync", font=("Arial", 12, "bold")
        ).pack(pady=10)

        # 1. View Logs Button
        self.btn_view = tk.Button(
            root, text="View Files on DIS", command=self.view_dis_files
        )
        self.btn_view.pack(pady=5, fill="x", padx=50)

        # 2. Download Button
        self.btn_download = tk.Button(
            root, text="Download All Logs", command=self.download_logs
        )
        self.btn_download.pack(pady=5, fill="x", padx=50)

        # 4. Clear DIS (Red Button)
        self.btn_clear = tk.Button(
            root, text="⚠️ Clear Logs on DIS", fg="red", command=self.clear_dis
        )
        self.btn_clear.pack(pady=20, fill="x", padx=50)

        # 5. Log Output Area
        tk.Label(root, text="Activity:", font=("Arial", 10, "bold")).pack(
            pady=(5, 0), anchor="w", padx=10
        )
        self.log_text = scrolledtext.ScrolledText(
            root, height=10, state="disabled", font=("Consolas", 9)
        )
    ...
    def view_dis_files(self):
        ...
        try:
            result = subprocess.run(
                ["mpremote", "fs", "ls", ":Logs/"], capture_output=True, text=True
            )
    ...
    def open_file_manager(self, files):
        ...
        tk.Label(top, text="Select files from Logs/", font=("Arial", 10, "bold")).pack(
            pady=5
        )
        ...
        def on_preview():
            ...
            if len(selected) > 1:
                messagebox.showwarning(
                    "Selection Error", "Please select only one file to preview."
                )
                return
        ...
        def on_download(delete=False):
            ...
            if delete and not messagebox.askyesno(
                "Confirm", f"Download and DELETE {len(selected)} files?"
            ):
                return
            top.destroy()
        ...
        def on_delete_selected():
            ...
            if messagebox.askyesno(
                "Confirm", f"Permanently delete {len(selected)} selected files?"
            ):
                top.destroy()
        ...
        def on_delete_all():
            if messagebox.askyesno(
                "Confirm", "Permanently delete ALL files in this list?"
            ):
                top.destroy()
                self.batch_delete(files)

        btn_preview = tk.Button(
            btn_frame, text="Preview Selected", command=on_preview, state="disabled"
        )
        ...
        tk.Button(
            btn_frame, text="Download Selected", command=lambda: on_download(False)
        ).pack(fill="x", pady=2)
        tk.Button(
            btn_frame,
            text="Download & Delete Selected",
            command=lambda: on_download(True),
        ).pack(fill="x", pady=2)
        tk.Button(
            btn_frame, text="Delete Selected", fg="red", command=on_delete_selected
        ).pack(fill="x", pady=2)
        tk.Button(
            btn_frame, text="Delete ALL Logs", fg="red", command=on_delete_all
    ...
    def preview_file(self, filename):
        ...
        try:
            ...
            result = subprocess.run(
                ["mpremote", "fs", "cat", f":Logs/{filename}"],
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
    ...
    def batch_process(self, files, delete):
        ...
        for filename in files:
            ...
            for attempt in range(3):
                try:
                    subprocess.run(
                        ["mpremote", "fs", "cp", remote_path, str(local_path)],
                        check=True,
                    )
                    success = True
                    break
            ...
            if success:
                if delete:
                    ...
                    try:
                        subprocess.run(
                            ["mpremote", "fs", "rm", remote_path], check=True
                        )
    ...
    def batch_delete(self, files):
        ...
        for filename in files:
            try:
                subprocess.run(
                    ["mpremote", "fs", "rm", f":Logs/{filename}"], check=True
                )
    ...
    def clear_dis(self):
        ...
        if not messagebox.askyesno(
            "Warning",
            "Are you sure you want to permanently delete all logs on the DIS?",
        ):
            return
